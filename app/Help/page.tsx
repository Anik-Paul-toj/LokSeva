...
// Municipalities Data (same as before)
...
const HelplinePage: React.FC = () => {
  ...
  const filteredMunicipalities = municipalitiesData.filter(entry =>
    ...
    <div className="min-h-screen" style={{ backgroundColor: theme.primary }}>
      <div className="max-w-7xl mx-auto px-4 py-8">
        ...
        <div className="mb-8 relative">
          ...
          />
          <svg
            className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-primary-text-light"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
            ...
            {filteredMunicipalities.map((entry, index) => (
              ...
              >
                ...
                <div className="space-y-4">
                  <div>
                    <h4 className="font-medium" style={{ color: theme.accent }}>Municipal Contact</h4>
                    <p className="text-sm" style={{ color: theme.textLight }}>
                      {formatContact(entry.municipalityContact)}
                    </p>
                  </div>
                  <div className="border-t pt-4">
                    <h4 className="font-medium" style={{ color: theme.accent }}>Police Station</h4>
                    <p className="text-sm" style={{ color: theme.textLight }}>
                      {entry.policeStation}
                    </p>
                    <p className="text-sm mt-1" style={{ color: theme.textLight }}>
                      {formatContact(entry.policeContact)}
                    </p>
                  </div>
                </div>
              </div>
        ...
        {/* Footer */}
        <footer className="mt-12 py-6 text-center rounded-xl" style={{ backgroundColor: theme.primaryDark }}>
          <p className="text-sm" style={{ color: theme.textLight }}>
            Official contacts maintained by West Bengal Government
          </p>
          <p className="text-xs mt-2" style={{ color: theme.textLight }}>
            Last verified: {new Date().toISOString().split('T')[0]} {/* YYYY-MM-DD format */}
          </p>
        </footer>
      </div>
    </div>
